...
# 1. Initialisieren von Eingangsbedingungen des Dataframes
...
# Initialisieren der Zeitstempel in Variable "time"
time = Battery_Dataframe['Zeit [s]']

# Initialisieren der Anfangs_temperatur
...
for i, row in Battery_Dataframe.iloc[1:].head(100).iterrows():
    ...
    u_zelle = init_volt     #"""HIER PUNKT 1 """
    #u_zelle = Battery_Dataframe.at[i - 1, 'U_ges [V]']
    ...
    Battery_Dataframe.at[i, 'SOC [%]'] = soc                                    #Einspeisen in das Dataframe
    ...
    r = Battery_Dataframe.at[i, 'R [Ohm]']                                      #(initialisieren der Variable)
    u_r = r * i_zelle                                                           #Gain Block oder Multiplikator
    Battery_Dataframe.at[i, 'U_R [V]'] = u_r                                    #Einspeisen in das Dataframe

    # 6.2: Berechnung von I_R1
    r_1 = Battery_Dataframe.at[i, 'R1 [Ohm]']                                   #(initialisieren der Variable)
    u_r1_prev = Battery_Dataframe.at[i - 1, 'U_R1 [V]']                         #(initialisieren der Variable)
    ...
    c_1 = Battery_Dataframe.at[i, 'C1 [C]']                                     #(initialisieren der Variable)
    ...
    Battery_Dataframe.at[i, 'U_R1 [V]'] = u_r1                                  #Einspeisen in das Dataframe

    # 6.5: Berechnung von I_R2
    r_2 = Battery_Dataframe.at[i, 'R2 [Ohm]']                                   #(initialisieren der Variable)
    u_r2_prev = Battery_Dataframe.at[i - 1, 'U_R2 [V]']                         #(initialisieren der Variable)
    ...
    c_2 = Battery_Dataframe.at[i, 'C2 [C]']                                     #(initialisieren der Variable)
    ...
    Battery_Dataframe.at[i, 'U_ges [V]'] = u_zelle                              #Einspeisen in das Dataframe
    ...
    Battery_Dataframe.at[i, 'Q_Irreversibel [W]'] = q_irreversibel              #Einspeisen in das Dataframe
    ...
    Battery_Dataframe.at[i, "Q_Cell [W]"] = q_zelle                             #Einspeisen in das Dataframe

    q_zelle_summe = anzahl_zellen * q_zelle                                     #Gain Block oder Multiplikator
    Battery_Dataframe.at[i, "Q_Sum_Cell [W]"] = q_zelle_summe                   #Einspeisen in das Dataframe
    ...
    ausgangs_temperatur_0 = Battery_Dataframe.at[i - 1, 'Output Temperature [K]']   #Temperatur aus letzter iteration
    ausgangs_temperatur_1 = ausgangs_temperatur_0 + d_temperatur * dt           #Integrator Block

    # Store this "output" temperature
    Battery_Dataframe.at[i, 'Output Temperature [K]'] = ausgangs_temperatur_1   #Einspeisen in das Dataframe

    if i == end_time - 1:  # Wenn wir die letzte Iteration erreichen
        Battery_Dataframe = Battery_Dataframe.iloc[:end_time]  # behalten Sie nur die Zeilen bis end_index


print('\nSimulation abgeschlossen!')

#plot(Battery_Dataframe.head(100), "Zeit [s]", 'Leistung [W]')
#plot(Battery_Dataframe.head(100), "Zeit [s]", 'Current [A]')
#plot(Battery_Dataframe.head(100), "Zeit [s]", 'Charge [C]')
plot(Battery_Dataframe.head(100), "Zeit [s]", 'SOC [%]')
#plot(Battery_Dataframe.head(100), "Zeit [s]", 'OCV [V]')
#plot(Battery_Dataframe.head(100), "Zeit [s]", 'R [Ohm]')
#plot(Battery_Dataframe.head(100), "Zeit [s]", 'R1 [Ohm]')
#plot(Battery_Dataframe.head(100), "Zeit [s]", 'R2 [Ohm]')
#plot(Battery_Dataframe.head(100), "Zeit [s]", 'C1 [C]')
